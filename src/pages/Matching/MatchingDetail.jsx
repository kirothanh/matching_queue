  ...
  useEffect(() => {
    ...
    socket.on("new_message", (data) => {
      const messageWithUserInfo = {
        ...data,
        sender: data.senderId === userValue?.id ? { ...userValue } : data.sender,
      };
        ...
        <div className="flex flex-col md:flex-row gap-2 scrollbar">
          ...
          <div className="w-full md:w-1/2 h-full flex-grow flex-col">
            <div className="bg-[#cbdbf1] rounded-lg h-full w-full flex-grow">
              ...
              {userValue?.id === listClub?.createdBy ? (
                <>
                  <div className="p-4 border-b">
                    <h2 className="text-xl font-semibold text-black">
                      Select Partner
          ...
          <div className="w-full bg-[#f5f7fa] flex flex-col h-[80vh] rounded-lg">
            ...
            >
              {messageList?.map((msg, index) => (
                <div
                  key={`${msg?.id}-${index}`}
                  className={`flex items-start gap-2 ${msg?.senderId === userValue?.id ? "justify-end" : ""
                    }`}
                >
                  {msg.senderId !== userValue?.id && (
                    <img
                  ...
                  {msg?.senderId === userValue?.id && (
                    <img
                      src={userValue?.avatar || "/img/avatar.png"}
                      alt="avatar"
                      className="rounded-full w-12 h-12 object-cover"
                    />
                  )}
